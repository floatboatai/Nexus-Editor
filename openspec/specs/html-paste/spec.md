# html-paste Specification

## Purpose

Convert pasted rich-text/HTML clipboard content into Markdown for `@floatboat/nexus-plugin-paste`: sanitize untrusted HTML at the trust boundary, keep converted link/image destinations and the surrounding alt/title text syntactically safe, route images through the host upload pipeline (the save-to-local default), and fall through cleanly for plain-text and file pastes.
...
- **WHEN** an `<img src="data:…">` carries a non-raster MIME (e.g. `data:text/html`, `data:image/svg+xml` — only `image/png`/`image/jpeg`/`image/gif`/`image/webp` are supported)
