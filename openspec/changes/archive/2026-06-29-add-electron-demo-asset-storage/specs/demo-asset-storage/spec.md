...
- **THEN** the main process SHALL generate the filename (timestamp + crypto-strong random) with the extension derived from a whitelist keyed on the MIME passed over IPC (`image/png`/`jpeg`/`gif`/`webp`)
